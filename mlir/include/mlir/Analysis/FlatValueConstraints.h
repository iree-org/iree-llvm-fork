//===- FlatConstraints.h - MLIR Affine Structures Class ----*- C++ -*-===//
//
...
#ifndef MLIR_ANALYSIS_FLATVALUECONSTRAINTS_H
#define MLIR_ANALYSIS_FLATVALUECONSTRAINTS_H
...
/// FlatConstraints is an extension of IntegerPolyhedron. It provides an
/// AffineExpr-based API.
class FlatConstraints : public presburger::IntegerPolyhedron {
public:
  ...
  FlatConstraints(unsigned numReservedInequalities,
  ...
  FlatConstraints(unsigned numDims = 0, unsigned numSymbols = 0,
                  unsigned numLocals = 0)
      : FlatConstraints(/*numReservedInequalities=*/0,
                        /*numReservedEqualities=*/0,
                        /*numReservedCols=*/numDims + numSymbols + numLocals +
                            1,
                        numDims, numSymbols, numLocals) {}

  FlatConstraints(const IntegerPolyhedron &fac) : IntegerPolyhedron(fac) {}

  /// Return the kind of this object.
  Kind getKind() const override { return Kind::FlatConstraints; }

  static bool classof(const IntegerRelation *cst) {
    return cst->getKind() >= Kind::FlatConstraints &&
           cst->getKind() <= Kind::FlatAffineRelation;
  }

  /// Clones this object.
  std::unique_ptr<FlatConstraints> clone() const;
...
/// FlatValueConstraints represents an extension of FlatConstraints where each
/// non-local variable can have an SSA Value attached to it.
class FlatValueConstraints : public FlatConstraints {
public:
  /// Constructs a constraint system reserving memory for the specified number
  /// of constraints and variables. `valArgs` are the optional SSA values
  /// associated with each dimension/symbol. These must either be empty or match
  /// the number of dimensions and symbols.
  FlatValueConstraints(unsigned numReservedInequalities,
                       unsigned numReservedEqualities, unsigned numReservedCols,
                       unsigned numDims, unsigned numSymbols,
                       unsigned numLocals,
                       ArrayRef<std::optional<Value>> valArgs)
      : FlatConstraints(numReservedInequalities, numReservedEqualities,
                        numReservedCols, numDims, numSymbols, numLocals) {
  ...
  /// dimension/symbol. These must either be empty or match the number of
  /// dimensions and symbols.
  FlatValueConstraints(unsigned numDims, unsigned numSymbols,
                       unsigned numLocals,
                       ArrayRef<std::optional<Value>> valArgs)
      : FlatValueConstraints(/*numReservedInequalities=*/0,
                             /*numReservedEqualities=*/0,
  ...
  /// dimension/symbol. These must either be empty or match the number of
  /// dimensions and symbols.
  FlatValueConstraints(unsigned numDims = 0, unsigned numSymbols = 0,
                       unsigned numLocals = 0, ArrayRef<Value> valArgs = {})
      : FlatValueConstraints(/*numReservedInequalities=*/0,
                             /*numReservedEqualities=*/0,
                             /*numReservedCols=*/numDims + numSymbols +
                                 numLocals + 1,
  ...
  FlatValueConstraints(const IntegerPolyhedron &fac,
                       ArrayRef<std::optional<Value>> valArgs = {})
      : FlatConstraints(fac) {
    assert(valArgs.empty() || valArgs.size() == getNumDimAndSymbolVars());
    if (valArgs.empty())
  ...
  explicit FlatValueConstraints(IntegerSet set, ValueRange operands = {});
  ...
  static FlatValueConstraints
  getHyperrectangular(ValueRange ivs, ValueRange lbs, ValueRange ubs);

  /// Return the kind of this object.
  Kind getKind() const override { return Kind::FlatValueConstraints; }

  static bool classof(const IntegerRelation *cst) {
    return cst->getKind() >= Kind::FlatValueConstraints &&
           cst->getKind() <= Kind::FlatAffineRelation;
  }

  /// Replaces the contents of this FlatValueConstraints with `other`.
  ...
  //
  void mergeAndAlignVarsWithOther(unsigned offset, FlatValueConstraints *other);
  ...
  /// other symbols.
  void mergeSymbolVars(FlatValueConstraints &other);
  ...
  bool areVarsAlignedWithOther(const FlatConstraints &other);
  ...
  LogicalResult unionBoundingBox(const FlatValueConstraints &other);
...
#endif // MLIR_ANALYSIS_FLATVALUECONSTRAINTS_H
