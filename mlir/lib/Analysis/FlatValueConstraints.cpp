...
// in FlatConstraints 'localVarCst'.
...
getFlattenedAffineExprs(ArrayRef<AffineExpr> exprs, unsigned numDims,
                        ...
                        FlatConstraints *localVarCst) {
  if (exprs.empty()) {
    if (localVarCst)
      *localVarCst = FlatConstraints(numDims, numSymbols);
    return success();
  }
...
LogicalResult mlir::getFlattenedAffineExpr(
    ...
    SmallVectorImpl<int64_t> *flattenedExpr, FlatConstraints *localVarCst) {
  std::vector<SmallVector<int64_t, 8>> flattenedExprs;
...
LogicalResult mlir::getFlattenedAffineExprs(
  ...
  if (map.getNumResults() == 0) {
    if (localVarCst)
      *localVarCst = FlatConstraints(map.getNumDims(), map.getNumSymbols());
    return success();
  }
...
LogicalResult mlir::getFlattenedAffineExprs(
  ...
  if (set.getNumConstraints() == 0) {
    if (localVarCst)
      *localVarCst = FlatConstraints(set.getNumDims(), set.getNumSymbols());
    return success();
  }
...
std::unique_ptr<FlatConstraints> FlatConstraints::clone() const {
  return std::make_unique<FlatConstraints>(*this);
}
...
LogicalResult FlatConstraints::composeMatchingMap(AffineMap other) {
...
static bool detectAsMod(const FlatConstraints &cst, unsigned pos,
  ...
  for (unsigned curEquality = 0, numEqualities = cst.getNumEqualities();
    ...
    if (quotientCount >= 1) {
      auto ub = cst.getConstantBound64(FlatConstraints::BoundType::UB,
                                       dimExpr.getPosition());
...
static bool detectAsFloorDiv(const FlatConstraints &cst, unsigned pos,
                             MLIRContext *context,
...
std::pair<AffineMap, AffineMap> FlatConstraints::getLowerAndUpperBound(
...
void FlatConstraints::getSliceBounds(unsigned offset, unsigned num,
                                     MLIRContext *context,
  ...
  std::optional<FlatConstraints> tmpClone;
  for (unsigned pos = 0; pos < num; pos++) {
    ...
    if (expr) {
      ...
      if (getNumLocalVars() == 0) {
        ...
        if (!tmpClone) {
          tmpClone.emplace(FlatConstraints(*this));
...
LogicalResult FlatConstraints::flattenAlignedMapAndMergeLocals(
    AffineMap map, std::vector<SmallVector<int64_t, 8>> *flattenedExprs) {
  FlatConstraints localCst;
...
LogicalResult FlatConstraints::addBound(BoundType type, unsigned pos,
                                        AffineMap boundMap,
                                        bool isClosedBound) {
...
LogicalResult
FlatConstraints::computeLocalVars(SmallVectorImpl<AffineExpr> &memo,
...
IntegerSet FlatConstraints::getAsIntegerSet(MLIRContext *context) const {
...
//===----------------------------------------------------------------------===//
// FlatValueConstraints
//===----------------------------------------------------------------------===//

// Construct from an IntegerSet.
FlatValueConstraints::FlatValueConstraints(IntegerSet set, ValueRange operands)
    : FlatConstraints(set.getNumInequalities(), set.getNumEqualities(),
                      set.getNumDims() + set.getNumSymbols() + 1,
                      set.getNumDims(), set.getNumSymbols(), /*numLocals=*/0) {
  // Populate values.
  if (operands.empty()) {
...
FlatValueConstraints FlatValueConstraints::getHyperrectangular(ValueRange ivs,
                                                               ValueRange lbs,
                                                               ValueRange ubs) {
  FlatValueConstraints res;
  ...
  for (int ivIdx = 0, e = nIvs; ivIdx < e; ++ivIdx) {
    ...
    if (failed(res.addBound(BoundType::LB, ivIdx, lb)))
      llvm_unreachable("Unexpected FlatValueConstraints creation error");
    // -iv + ub >= 0
...
unsigned FlatValueConstraints::appendDimVar(ValueRange vals) {
  unsigned pos = getNumDimVars();
  return insertVar(VarKind::SetDim, pos, vals);
}

unsigned FlatValueConstraints::appendSymbolVar(ValueRange vals) {
  unsigned pos = getNumSymbolVars();
  return insertVar(VarKind::Symbol, pos, vals);
}

unsigned FlatValueConstraints::insertDimVar(unsigned pos, ValueRange vals) {
  return insertVar(VarKind::SetDim, pos, vals);
}

unsigned FlatValueConstraints::insertSymbolVar(unsigned pos, ValueRange vals) {
  return insertVar(VarKind::Symbol, pos, vals);
}

unsigned FlatValueConstraints::insertVar(VarKind kind, unsigned pos,
                                         unsigned num) {
...
bool FlatValueConstraints::hasValues() const {
  return llvm::any_of(
...
static bool areVarsAligned(const FlatValueConstraints &a,
                           const FlatValueConstraints &b) {
...
bool FlatValueConstraints::areVarsAlignedWithOther(
    const FlatConstraints &other) {
  return areVarsAligned(*this, other);
}

/// Checks if the SSA values associated with `cst`'s variables in range
/// [start, end) are unique.
static bool LLVM_ATTRIBUTE_UNUSED areVarsUnique(const FlatValueConstraints &cst,
...
areVarsUnique(const FlatValueConstraints &cst) {
...
static void mergeAndAlignVars(unsigned offset, FlatValueConstraints *a,
                              FlatValueConstraints *b) {
...
void FlatValueConstraints::mergeAndAlignVarsWithOther(
    unsigned offset, FlatValueConstraints *other) {
...
/// other symbols.
void FlatValueConstraints::mergeSymbolVars(FlatValueConstraints &other) {
...
bool FlatValueConstraints::hasConsistentState() const {
  return IntegerPolyhedron::hasConsistentState() &&
         values.size() == getNumDimAndSymbolVars();
}

void FlatValueConstraints::removeVarRange(VarKind kind, unsigned varStart,
                                          unsigned varLimit) {
...
AffineMap FlatValueConstraints::computeAlignedMap(AffineMap map,
...
bool FlatValueConstraints::findVar(Value val, unsigned *pos) const {
  unsigned i = 0;
...
bool FlatValueConstraints::containsVar(Value val) const {
  return llvm::any_of(values, [&](const std::optional<Value> &mayBeVar) {
    return mayBeVar && *mayBeVar == val;
  });
}

void FlatValueConstraints::swapVar(unsigned posA, unsigned posB) {
...
void FlatValueConstraints::addBound(BoundType type, Value val, int64_t value) {
  unsigned pos;
  if (!findVar(val, &pos))
...
void FlatConstraints::printSpace(raw_ostream &os) const {
...
void FlatValueConstraints::printSpace(raw_ostream &os) const {
  IntegerPolyhedron::printSpace(os);
  os << "(";
  for (unsigned i = 0, e = getNumDimAndSymbolVars(); i < e; i++) {
    if (hasValue(i))
...
void FlatValueConstraints::clearAndCopyFrom(const IntegerRelation &other) {

  if (auto *otherValueSet = dyn_cast<const FlatValueConstraints>(&other)) {
    *this = *otherValueSet;
  } else {
...
void FlatValueConstraints::fourierMotzkinEliminate(unsigned pos,
                                                   bool darkShadow,
...
void FlatValueConstraints::projectOut(Value val) {
  unsigned pos;
...
LogicalResult
FlatValueConstraints::unionBoundingBox(const FlatValueConstraints &otherCst) {
  ...
  if (!areVarsAligned(*this, otherCst)) {
    FlatValueConstraints otherCopy(otherCst);
