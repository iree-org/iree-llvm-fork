...
static Value foldExtractFromBroadcast(ExtractOp extractOp) {
  ...
  if (hasZeroDimVectors(extractOp) || hasZeroDimVectors(defOp))
    return Value();
...
static Value foldExtractFromShapeCast(ExtractOp extractOp) {
  ...
  // 0-D vectors not supported.
  if (hasZeroDimVectors(extractOp) || hasZeroDimVectors(shapeCastOp))
    return Value();
...
static Value foldExtractFromExtractStrided(ExtractOp extractOp) {
  ...
  // 0-D vectors not supported.
  if (hasZeroDimVectors(extractOp) || hasZeroDimVectors(extractStridedSliceOp))
    return Value();
...
static Value foldExtractStridedOpFromInsertChain(ExtractOp extractOp) {
  ...
  // 0-D vectors not supported.
  if (hasZeroDimVectors(extractOp) || hasZeroDimVectors(insertOp))
    return Value();
