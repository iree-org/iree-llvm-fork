...
createContractArithOp(Location loc, Value x, Value y, Value acc,
  ...
  if (isInt) {
    ...
    if (acc && acc.getType().isa<VectorType>() && kind == CombiningKind::ADD) {
      ...
      if (mask)
        // The fma op doesn't need masking but for reductions, we have to make
        // sure that the previous 'acc' value is passed thru for the masked-out
        // lanes.
        fma = selectPassthruWithMask(rewriter, mask, fma, acc);
      return fma;
    }
...
struct ContractOpToElementwise
  ...
  LogicalResult matchAndRewrite(vector::ContractionOp contractOp,
    ...
    assert(!maskableOp.isMasked() && "ToElementwise");
...
ContractionOpToMatmulOpLowering::matchAndRewrite(vector::ContractionOp op,
  ...
  assert(!maskableOp.isMasked() && "MatmulOp");
...
ContractionOpToDotLowering::matchAndRewrite(vector::ContractionOp op,
  ...
  assert(!maskableOp.isMasked() && "DotLowering");
...
FailureOr<Value> ContractionOpLowering::lowerParallel(PatternRewriter &rewriter,
  ...
  for (int64_t d = 0; d < dimSize; ++d) {
    ...
    reshapeStore(loc, lowContract->getResult(0), result, resType, resIndex, d,
                 rewriter);
  }
  return result;
}
