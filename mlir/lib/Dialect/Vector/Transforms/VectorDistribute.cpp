...
struct WarpOpScfForOp : public OpRewritePattern<WarpExecuteOnLane0Op> {
  ...
  LogicalResult matchAndRewrite(WarpExecuteOnLane0Op warpOp,
    ...
    forOp.getBodyRegion().walk([&](Operation *op) {
      for (Value operand : op->getOperands()) {
        Operation *parent = operand.getParentRegion()->getParentOp();
        if (forOp->isAncestor(parent))
          continue;
        if (warpOp->isAncestor(parent)) {
          if (!escapingValues.insert(operand))
            continue;
          Type distType = operand.getType();
          if (auto vecType = distType.cast<VectorType>()) {
            AffineMap map = distributionMapFn(operand);
