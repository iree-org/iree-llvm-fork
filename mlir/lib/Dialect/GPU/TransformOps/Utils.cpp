...
static GpuIdBuilderFnType common3DIdBuilderFn(int64_t multiplicity = 1) {
  auto res = [multiplicity](RewriterBase &rewriter, Location loc,
    ...
    return IdBuilderResult{ids, SmallVector<int64_t>{forallMappingSizes},
                           SmallVector<int64_t>{originalBasis}, ids};
  };
  return res;
}
...
GpuWarpgroupIdBuilder::GpuWarpgroupIdBuilder(MLIRContext *ctx,
...
GpuWarpIdBuilder::GpuWarpIdBuilder(MLIRContext *ctx, bool useLinearMapping)
    : GpuIdBuilder(ctx, useLinearMapping, [](MLIRContext *ctx, MappingId id) {
        return GPUWarpMappingAttr::get(ctx, id);
      }) {
  idBuilder =
      useLinearMapping
          ? commonLinearIdBuilderFn<ThreadIdOp>(/*multiplicity=*/kWarpSize)
