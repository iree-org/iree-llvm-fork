...
struct Conv1DGenerator
  ...
  Value promote(RewriterBase &rewriter, Location loc, Value val, Type ty) {
    ...
    const Type dstType = val.getType().cast<ShapedType>().cloneWith(
        std::nullopt, dstElementType);

    if (dstElementType.isa<FloatType>() && srcWidth < dstWidth)
      return rewriter.create<arith::ExtFOp>(loc, dstType, val);

    if (dstElementType.isa<IntegerType>() && srcWidth < dstWidth)
