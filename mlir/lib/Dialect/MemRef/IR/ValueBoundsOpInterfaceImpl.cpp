...
struct AllocOpInterface
  ...
  void populateBoundsForShapedValueDim(Operation *op, Value value, int64_t dim,
    ...
    cstr.addBound(BoundType::EQ, value, dim,
...
struct DimOpInterface
  ...
  void populateBoundsForIndexValue(Operation *op, Value value,
    ...
    if (!constIndex.has_value())
      return;
    cstr.addBound(BoundType::EQ, value,
...
struct RankOpInterface
  ...
  void populateBoundsForIndexValue(Operation *op, Value value,
    ...
    if (!memrefType)
      return;
    cstr.addBound(BoundType::EQ, value, cstr.getExpr(memrefType.getRank()));
  }
};
  ...
  void populateBoundsForShapedValueDim(Operation *op, Value value, int64_t dim,
    ...
    for (int64_t i = 0, e = subViewOp.getMixedSizes().size(); i < e; ++i) {
      ...
      if (!dropped.test(i))
        ++ctr;
      if (ctr == dim) {
        cstr.addBound(BoundType::EQ, value, dim, subViewOp.getMixedSizes()[i]);
        return;
      }
    }
